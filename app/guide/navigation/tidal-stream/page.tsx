...
function ResultCard({
  ...
  const styles = {
    ...
    green: "border-green-300/25 bg-green-300/10 text-green-300",
    purple: "border-purple-300/25 bg-purple-300/10 text-purple-300",
    red: "border-red-300/25 bg-red-300/10 text-red-300",
  };
  ...
  return (
    <div className={`rounded-2xl border p-5 ${styles[color]}`}>
      ...
      <div className="mt-3 flex items-end gap-2">
        ...
        {suffix ? <span className="pb-1 text-sm font-black">{suffix}</span> : null}
      </div>
    </div>
  );
}
  ...
  return (
    <main className="min-h-screen bg-[#020817] text-white">
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_12%,rgba(34,211,238,0.16),transparent_34%),radial-gradient(circle_at_82%_22%,rgba(59,130,246,0.14),transparent_35%)]" />
        <div className="absolute inset-0 opacity-[0.08] bg-[linear-gradient(rgba(34,211,238,0.16)_1px,transparent_1px),linear-gradient(90deg,rgba(34,211,238,0.16)_1px,transparent_1px)] bg-[size:38px_38px]" />
        ...
        <div className="relative mx-auto max-w-7xl px-5 py-8 md:px-8">
          ...
          <div className="mt-7 grid gap-6 lg:grid-cols-[1.45fr_0.75fr]">
            <div className="rounded-[2rem] border border-cyan-300/25 bg-slate-950/75 p-5 shadow-2xl shadow-cyan-950/30">
              ...
              <div className="relative overflow-hidden rounded-[2rem] border border-cyan-300/20 bg-slate-900/70">
                ...
                <svg viewBox="0 0 620 620" className="relative w-full">
                  <circle cx="310" cy="310" r="280" fill="rgba(2,8,23,0.55)" stroke="rgba(34,211,238,0.28)" strokeWidth="2" />
                  ...
                  <text x="310" y="34" textAnchor="middle" fill="#ef4444" fontSize="32" fontWeight="900">N</text>
                  <text x="586" y="316" textAnchor="middle" fill="#22d3ee" fontSize="26" fontWeight="900">E</text>
                  <text x="310" y="594" textAnchor="middle" fill="#ef4444" fontSize="32" fontWeight="900">S</text>
                  <text x="34" y="316" textAnchor="middle" fill="#e2e8f0" fontSize="26" fontWeight="900">W</text>
            ...
            <aside className="grid gap-5">
              ...
              <div className="rounded-[2rem] border border-cyan-300/25 bg-slate-950/80 p-5">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-2">
                  ...
                  <ResultCard title="CTS" value={result.attainable ? `${formatDeg(result.cts)}°` : "No safe CTS"} color="purple" />
                </div>
                ...
                <div className="mt-4 rounded-2xl border border-green-300/25 bg-green-300/10 p-4">
          ...
          <div className="mt-5 grid overflow-hidden rounded-2xl border border-cyan-300/25 bg-slate-950/75 md:grid-cols-5">
            <ResultCard title="Boat Distance" value={result.boatDistance.toFixed(2)} suffix="NM" color="cyan" icon={<Waves className="h-5 w-5" />} />
            <ResultCard title="Drift" value={result.drift.toFixed(2)} suffix="NM" color="amber" />
            <ResultCard title="COG" value={`${formatDeg(result.cog)}°`} color="green" />
            <ResultCard title="SOG" value={result.sog.toFixed(2)} suffix="kt" color="cyan" icon={<Gauge className="h-5 w-5" />} />
            <ResultCard title="CTS" value={`${formatDeg(result.cts)}°`} color="purple" icon={<Navigation className="h-5 w-5" />} />
          </div>
          ...
          <div className="mt-5 rounded-2xl border border-amber-300/25 bg-amber-300/10 p-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm leading-7 text-amber-100">
                ...
                pilot book, güncel hava, derinlik, emniyet marjı ve görsel/radar
                doğrulaması birlikte değerlendirilmelidir.
              </p>
            </div>
          </div>
        </div>
